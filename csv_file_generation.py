        ...
        with st.expander("Combined Data Table & Spectrum Viewer", expanded=False):
            ...
            if combined_data:
                ...
                with cols[1]:
                    st.info(
                        f"**Position:** "
                        f"({selected_data['x_position_mm']:.3f}, {selected_data['y_position_mm']:.3f}) mm"
                    )
                ...
                for i, layer_data in enumerate(selected_layers):
                    if i + 2 < len(cols):
                        with cols[i + 2]:
                            thickness_str = (
                                f"{layer_data['thickness_nm']:.2f} nm"
                                if layer_data['thickness_nm'] else "N/A"
                            )
                ...
                try:
                    fig = plot_spectrum(
                        selected_data,
                        f"Spectrum: {selected_file}",
                        yscale=yscale
                    )
                    st.plotly_chart(fig, use_container_width=True)

                except Exception as e:
                    st.error("Failed to render spectrum.")
                    st.exception(e)
