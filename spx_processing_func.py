                    ...
                    if not spx_folder.exists():
                        ...
                        if len(spx_files) == 0:
                            ...
                            try:
                                ...
                                st.success(
                                    f"Processed {len(combined_data)} files successfully! ({matched_count} matched)")
        ...
        for layer_name, processed in all_results.items():
            ...
            with st.expander("XRF Excel Data & Combined Data Table", expanded=False):
                ...
                if combined_data:
                    ...
                    with col1:
                        st.info(
                            f"**Position:** ({selected_data['x_position_mm']:.3f}, {selected_data['y_position_mm']:.3f}) mm")
                    with col2:
                        st.info(f"**Thickness:** {selected_data['thickness_nm']:.2f} nm" if selected_data[
                            'thickness_nm'] else "**Thickness:** N/A")
                    with col3:
